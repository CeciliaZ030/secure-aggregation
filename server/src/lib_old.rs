...
//Buffer size
	...
	pub fn workflow(&mut self, identity: Vec<u8>, msg: Vec<String>) -> Result<u32, &str>
	{
		...
		match msg[0].as_str() {
    		...
    		"KE" => {
                assert!(self.client_list.contains(&identity));
    		...
    		"IS" => {
                ...
                self.send("Send your share", &identity);
                while self.take_id() != identity {
                    unimplemented!();
                }

                for (client_id, data) in self.client_data.iter() {
                    println!("IS {:?}", String::from_utf8(self.take_id()).unwrap());
                    // assert_eq!(&self.take_id(), &identity);
                    let share = self.recv_vecs();
                    self.send_vec(share, client_id.to_vec());
                }
                self.send("Finished fowarding", &identity);
    			Ok(1)
    		},
    		_ => {
    			panic!("What the heck??")
...
pub fn client_data_to_vector(datas: &HashMap<Vec<u8>, ClientData>, 
    order: &Vec<Vec<u8>>, field: &str) -> Vec<Vec<u8>> {
